...
const GallerySlider = ({ clients }) => {
  ...
  const updateSlider = () => {
    ...
    slidesRefs.forEach((slide, index) => {
      ...
      const y =
        translateBreakPoint * Math.tan((distancePercentage * Math.PI) / 4); //arc equation
    ...
    <div className="slider-container">
      ...
      >
        <div className="slider-inner" ref={sliderInner}>
          <div className="slide empty"></div>
          {clients.map((client, i) => (
            <div
              className="slide text-center justify-center"
              key={i}
              ref={slidesRefs[i]}
            >
              <img
                ...
                alt={`Image ${i}`}
              />
              <button className={`drag-btn ${isDragging ? "hide" : ""}`}>
                Drag
              </button>
